...
class NoPropCT(nn.Module):
    def __init__(
        self, backbone: str, num_classes: int, time_emb_dim: int, embed_dim: int
    ):
        super().__init__()
...
# ----------------------------------------------------------------------------
@torch.no_grad()
def run_noprop_ct_inference(
    model: NoPropCT, x: torch.Tensor, T_steps: int = 1000
) -> torch.Tensor:
    model.eval()
    B = x.size(0)
...
def run_noprop_ct_inference_heun(
    model: NoPropCT, x: torch.Tensor, T_steps: int = 40
) -> torch.Tensor:
    model.eval()
    B = x.size(0)
    embed_dim = model.W_embed.size(1)
...
def train_and_eval(
    ...
    print(
        f"--- {backbone} on {dataset.upper()} ({num_classes} classes) using {device} ---"
    )

    print("dataset config")
    tr_loader = DataLoader(
        ds_train, batch_size=2048, shuffle=True, num_workers=8, drop_last=True
    )
    ...
    for ep in range(1, epoches + 1):
        ...
        if ep % 5 == 0:
            ...
            for x, y in te_loader:
                ...
                tot += y.size(0)
            print(
                f" | Acc {100 * corr / tot:4.2f}% | infer {time.time() - eval_t0:3.1f}s",
                end="",
            )
        print()
    ...
    for T in [2, 5, 10, 20, 40, 80, 100]:
        ...
        for x, y in te_loader:
            ...
            tot += y.size(0)
        print(
            f"Heun T={T:3d} acc {100 * corr / tot:4.2f}% | infer {time.time() - ti:3.1f}s"
        )
    ...
    for T in [2, 5, 10, 20, 40, 80, 100]:
        ...
        for x, y in te_loader:
            ...
            tot += y.size(0)
        print(
            f"Euler T={T:3d} acc {100 * corr / tot:4.2f}% | infer {time.time() - ti:3.1f}s"
        )
...
def main():
    ...
    parser.add_argument(
        "--backbone", choices=["resnet18", "resnet50", "resnet152"], required=True
    )
