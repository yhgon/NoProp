...
class ZEncoder(nn.Module):
    def __init__(self, num_classes: int, embed_dim: int = 256):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(num_classes, embed_dim), nn.ReLU())
...
class TEncoder(nn.Module):
    def __init__(self, time_emb_dim: int = 64, embed_dim: int = 256):
        ...
        self.net = nn.Sequential(nn.Linear(time_emb_dim, embed_dim), nn.ReLU())
...
def run_noprop_ct_inference(
    model: nn.Module, x: torch.Tensor, T_steps: int = 1000
) -> torch.Tensor:
    model.eval()
    B = x.size(0)
    m = model.fuse[-1].out_features
...
def run_noprop_ct_inference_heun(
    model: nn.Module, x: torch.Tensor, T_steps: int = 40
) -> torch.Tensor:
    model.eval()
    B = x.size(0)
    m = model.fuse[-1].out_features
...
def train_and_eval(backbone: str):
    ...
    # model and optimizer
    model = NoPropCT(backbone, num_classes=10, time_emb_dim=64, embed_dim=256).to(
        device
    )
